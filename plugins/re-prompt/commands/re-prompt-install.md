...
If `re-prompt --version` prints `0.2.2` and `re-prompt doctor` succeeds, report setup complete in a short summary. Only inspect repository docs or plugin files if one of these checks fails.
...
Preferred after the `v0.2.2` GitHub Release exists:

```bash
npm install -g https://github.com/kio-vibe/re-prompt/releases/download/v0.2.2/re-prompt-0.2.2.tgz
```
